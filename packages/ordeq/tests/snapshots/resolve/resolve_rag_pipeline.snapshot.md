...
runnables = [importlib.import_module("example_rag_pipeline")]
...
[('example_rag_pipeline.rag.annotation',
  'annotate_documents',
  Node(name=example_rag_pipeline.rag.annotation:annotate_documents, inputs=[IO(idx=ID2), IO(idx=ID6)], outputs=[IO(idx=ID7)])),
 ('example_rag_pipeline.rag.evaluation',
  'evaluate_answers',
  Node(name=example_rag_pipeline.rag.evaluation:evaluate_answers, inputs=[IO(idx=ID2), IO(idx=ID3)], outputs=[IO(idx=ID5)])),
 ('example_rag_pipeline.rag.indexer',
  'create_vector_index',
  Node(name=example_rag_pipeline.rag.indexer:create_vector_index, inputs=[IO(idx=ID6), IO(idx=ID4)], outputs=[IO(idx=ID1)])),
 ('example_rag_pipeline.rag.policies',
  'generate_questions',
  Node(name=example_rag_pipeline.rag.policies:generate_questions, inputs=[IO(idx=ID8)], outputs=[IO(idx=ID9)])),
 ('example_rag_pipeline.rag.question_answering',
  'question_answering',
  Node(name=example_rag_pipeline.rag.question_answering:question_answering, inputs=[IO(idx=ID9), IO(idx=ID10), IO(idx=ID3)], outputs=[IO(idx=ID2)])),
 ('example_rag_pipeline.rag.retrieval',
  'filter_relevant',
  Node(name=example_rag_pipeline.rag.retrieval:filter_relevant, inputs=[IO(idx=ID11), IO(idx=ID3)], outputs=[IO(idx=ID10)])),
 ('example_rag_pipeline.rag.retrieval',
  'retrieve',
  Node(name=example_rag_pipeline.rag.retrieval:retrieve, inputs=[IO(idx=ID1), IO(idx=ID9), IO(idx=ID4)], outputs=[IO(idx=ID11)]))]
