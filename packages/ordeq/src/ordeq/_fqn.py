...
def is_object_ref(obj: object) -> TypeGuard[ObjectRef]:
...
class FQN(NamedTuple):
    ...
    def from_ref(cls, ref: ObjectRef) -> FQN:
        """Convert an object reference to a fully qualified name (FQN).

        Args:
            ref: A string in the format "module:name".

        Returns:
            A named tuple representing the fully qualified name
                (module, name).
        ...
        return FQN(module=module_name, name=obj_name)
