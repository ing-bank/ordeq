...
- we defined a placeholder IO `txs_filtered` in `catalog.py`. This IO is not associated with any actual data source or sink. It merely serves to pass the output from one node to the other.
...
=== "nodes.py"
    ...
    @view(inputs=[catalog.txs, catalog.date])
