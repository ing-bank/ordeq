...
nodes, ios = _scan_fqns(*_resolve_packages_to_modules(example_rag_pipeline))
print("Nodes:")
pp(nodes, width=40)
print("IOs:")
...
```text
Nodes:
{Node(module=example_rag_pipeline.rag.annotation, name=annotate_documents, inputs=[IO(id=ID1), IO(id=ID2)], outputs=[IO(id=ID3)]): [FQN(module='example_rag_pipeline.rag.annotation', name='annotate_documents')],
